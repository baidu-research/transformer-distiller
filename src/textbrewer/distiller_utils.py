...
class accumulated_loss:
    """
    Averge the losses in each (sub)step of gradient accumulation
    """
    def __init__(self):
        self.accumulated_loss = 0.
        self.accumulated_losses_dict = {}
        self.count = 0

    def add(self, new_loss, new_losses_dict):
        self.accumulated_loss += new_loss
        for k, v in new_losses_dict.items():
            if k in self.accumulated_losses_dict:
                self.accumulated_losses_dict[k] += v
            else:
                self.accumulated_losses_dict[k] = v
        self.count += 1
    ...
    def report(self):
        return self.accumulated_loss/self.count, \
               {k: v/self.count for k, v in self.accumulated_losses_dict.items()}
...
class AbstractDistiller(DistillationContext):
    def __init__(self, train_config: TrainingConfig,
        ...
        self.rank = self.t_config.rank 
        self.world_size = self.t_config.world_size
...
class progress_meter:
    """
        Record number of training steps
    """
    def __init__(self):
        self.global_step = 0
